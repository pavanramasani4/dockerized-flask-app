import pytest
from WebPage import Week1
from unittest.mock import patch
from flask import url_for

@pytest.fixture
def client():
    Week1.application.config['TESTING'] = True
    with Week1.application.test_client() as client:
        yield client
...
def test_login(client):
    with patch('WebPage.Week1.get_db_connection') as mock_get_db:
        ...
        mock_conn.execute.return_value.fetchone.return_value = {
            'username': 'test_user',
            'password': Week1.generate_password_hash('test_password')
        }
