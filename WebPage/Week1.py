...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            session['start_time'] = datetime.datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S"
...
def display_users():
    ...
    return render_template('users.html', users=users)  # Render users.html with user data
...
def homepage():
    ...
    return render_template('home.html', username=username)  # Render home.html template
...
def logout():
    ...
    with get_db_connection() as conn:
        conn.execute(
            'UPDATE users SET session_end = ? WHERE username = ?',
            (end_time, username)
        )
        conn.commit()
...
# Route for serving the coverage report
@webpage.route('/coverage')
def coverage_report():
    return redirect(url_for('static', filename='htmlcov/index.html'))  # Redirect to coverage report

# Ensure the application instance is named 'application' for deployment
application = webpage

if __name__ == '__main__':
    application.run(host='0.0.0.0', port=5000, debug=True)  # Run the app
